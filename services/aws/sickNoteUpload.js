...
/**
 * Upload a sick note PDF to S3 with secure, organized storage
 ...
 */
async function uploadSickNote(patientId, file) {
  try {
    // Validate inputs
    if (!patientId) {
    ...
    if (file.mimetype !== "application/pdf") {
      throw new Error("Only PDF files are allowed for sick notes");
    }
    ...
    // Create organized folder structure: patients/{patientId}/sick-notes/{filename}
    const key = `patients/${patientId}/sick-notes/${filename}`;
    ...
    console.log("Uploading sick note to S3:", {
    ...
    console.error("Sick note upload error:", error);
...
/**
 * Generate a presigned URL for secure access to a sick note
...
async function generatePresignedUrl(key, expiresIn = 3600) {
  try {
    console.log("Generating presigned URL for sick note:", {
      key,
      expiresIn,
      bucketName,
      region,
    ...
    console.log("Sick note presigned URL generated successfully");
    return presignedUrl;
  } catch (error) {
    console.error("Error generating presigned URL for sick note:", error);
...
/**
 * Delete a sick note from S3
 * @param {string} key - The S3 key of the file to delete
 * @returns {Promise<Object>} Deletion result
 */
async function deleteSickNote(key) {
  try {
